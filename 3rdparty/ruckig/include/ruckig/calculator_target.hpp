...
namespace ruckig
{
    ...
    private:
        ...
        bool is_input_collinear( const InputParameter< DOFs >& inp, const Vector< double >& jMax, Profile::Direction limiting_direction, size_t limiting_dof, Vector< double >& new_max_jerk )
        {
            ...
            if ( !pd_found_nonzero )
            {  // position difference is zero everywhere...
                return false;
            }
            ...
            for ( size_t dof = 0; dof < pd.size( ); ++dof )
            {
                ...
                if (
                    std::abs( inp.current_velocity[ dof ] - v0_scale * pd[ dof ] ) > eps_colinear || std::abs( inp.current_acceleration[ dof ] - a0_scale * pd[ dof ] ) > eps_colinear || std::abs( inp.target_velocity[ dof ] - vf_scale * pd[ dof ] ) > eps_colinear || std::abs( inp.target_acceleration[ dof ] - af_scale * pd[ dof ] ) > eps_colinear )
                {
                    return false;
                }
