...
namespace KDL
{
    Path_Circle::Path_Circle( const Frame& F_base_start,
                              ...
                              double _eqradius,
                              bool _aggregate ) : orient( _orient ),
                                                  eqradius( _eqradius ),
                                                  aggregate( _aggregate )
    {
        ...
        Vector x( F_base_start.p - F_base_center.p );  //** 圆弧起点到圆心的向量 **//
        radius = x.Normalize( );
        if ( radius < epsilon )
        {
            if ( aggregate )
        ...
        Vector tmpv( V_base_p - F_base_center.p );  //** 圆弧末端到圆心的向量 **//
        tmpv.Normalize( );
        Vector z( x * tmpv );
        ...
        F_base_center.M = Rotation( x, z * x, z );  //** 圆心的旋转矩阵 **//
