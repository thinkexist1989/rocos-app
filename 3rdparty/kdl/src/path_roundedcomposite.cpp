...
namespace KDL
{
    ...
    Path_RoundedComposite::Path_RoundedComposite(Path_Composite *_comp,
                                                 ...
                                                 bool _aggregate, int _nrofpoints) : comp(_comp), radius(_radius), eqradius(_eqradius), orient(_orient), nrofpoints(_nrofpoints), aggregate(_aggregate)
    {
    } //** _radius取值越大，在算出来的d值越大 **//

    Path_RoundedComposite::Path_RoundedComposite(double _radius, double _eqradius, RotationalInterpolation *_orient, bool _aggregate) : comp(new Path_Composite()), radius(_radius), eqradius(_eqradius), orient(_orient), aggregate(_aggregate)
    {
        nrofpoints = 0;
        if (eqradius <= 0)
        {
