...
const auto action_start = [](rocos::Executor& robot) {
    robot.on_fsm_start();
};
const auto action_run = [](rocos::Executor& robot) {
    robot.on_fsm_run();
};
const auto action_pause = [](rocos::Executor& robot) {
    robot.on_fsm_pause();
};
const auto action_continue = [](rocos::Executor& robot) {
    robot.on_fsm_continue();
};
const auto action_stop = [](rocos::Executor& robot) {
    robot.on_fsm_stop();
};
const auto action_reset = [](rocos::Executor& robot) {
    robot.on_fsm_reset();
};
const auto action_enable = [](rocos::Executor& robot) {
    robot.on_fsm_enable();
};
const auto action_disable = [](rocos::Executor& robot) {
    robot.on_fsm_disable();
};
const auto action_identify = [](rocos::Executor& robot) {
...
namespace rocos {
    class Executor::Impl {
    public:
        explicit Impl(Executor& owner) : sm_ {owner} {}
    ...
    Executor::Executor() : impl_(std::make_unique<Impl>(*this)) {
    ...
    std::string Executor::GetRobotState() const {
    ...
    bool Executor::IsDisabled() {

        return true;
    }

    bool Executor::isMotionRunning() const {
        return impl_->is(sml::state<class RUNNING>);
    }

    void Executor::waitControlCycle() {

    }

    void Executor::initializeMotionExecutor() {

    }

    void Executor::setEnabled() {
    }

    void Executor::setDisabled() {
    }
