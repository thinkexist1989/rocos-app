...
class EcatConfig {
 ...
 public:
  ...
  void waitForSignal(
      int id = 0);  // compact code, not recommended use. use wait() instead
  ...
  T getSlaveInputVarValue(int slaveId, int varId) {
    ...
    return *(T *)((char *)pdInputPtr +
                  ecatBus->slaves[slaveId].input_vars[varId].offset);
  }
  ...
  void setSlaveInputVarValue(int slaveId, int varId, T value) {
    ...
    *(T *)((char *)pdInputPtr +
           ecatBus->slaves[slaveId].input_vars[varId].offset) = value;
  }
  ...
  T getSlaveOutputVarValue(int slaveId, int varId) {
    ...
    return *(T *)((char *)pdOutputPtr +
                  ecatBus->slaves[slaveId].output_vars[varId].offset);
  }
  ...
  template <typename T>
  void setSlaveInputVarValueByName(int slaveId, const std::string &varName,
                                   T value) {
    for (int i = 0; i < ecatBus->slaves[slaveId].input_var_num; ++i) {
      if (strcmp(ecatBus->slaves[slaveId].input_vars[i].name,
                 varName.c_str()) == 0) {
        ...
        *(T *)((char *)pdInputPtr +
               ecatBus->slaves[slaveId].input_vars[i].offset) = value;
      }
    }
  }
  ...
  T getSlaveOutputVarValueByName(int slaveId, const std::string &varName) {
    for (int i = 0; i < ecatBus->slaves[slaveId].output_var_num; ++i) {
      if (strcmp(ecatBus->slaves[slaveId].output_vars[i].name,
                 varName.c_str()) == 0) {
        if (sizeof(T) != ecatBus->slaves[slaveId].output_vars[i].size) {
          print_message("Size of Var is not equal", MessageLevel::WARNING);
        }
        return *(T *)((char *)pdOutputPtr +
                      ecatBus->slaves[slaveId].output_vars[i].offset);
      }
    }
    return std::numeric_limits<T>::max();
  }
  ...
  template <typename T>
  void setSlaveOutputVarValueByName(int slaveId, const std::string &varName,
                                    T value) {
    for (int i = 0; i < ecatBus->slaves[slaveId].output_var_num; ++i) {
      if (strcmp(ecatBus->slaves[slaveId].output_vars[i].name,
                 varName.c_str()) == 0) {
        ...
        *(T *)((char *)pdOutputPtr +
               ecatBus->slaves[slaveId].output_vars[i].offset) = value;
      }
    }
  }
 ...
 private:
  ...
  enum MessageLevel { NORMAL = 0, WARNING = 1, ERROR = 2 };
  ...
  void print_message(const std::string &msg,
                     MessageLevel msgLvl = MessageLevel::NORMAL);
};
}  // namespace rocos
