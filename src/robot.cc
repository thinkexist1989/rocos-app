...
//        addAllJoints( ); // TODO: 这个应该直接加到参数解析里面，解析之后加入关节，顺序和主站顺序可能不一样
    ...
    int Robot::Dragging(DraggingFlag flag, DraggingDirection dir, double max_speed, double max_acceleration) {
        ...
        switch (flag) {
            case DraggingFlag::J0:
            case DraggingFlag::J1:
            case DraggingFlag::J2:
            case DraggingFlag::J3:
            case DraggingFlag::J4:
            case DraggingFlag::J5:
            case DraggingFlag::J6:
                ...
                if (_dragging_finished_flag) {
                    ...
                    thread_planning.reset(
                            new boost::thread{&JC_helper::smart_servo::smart_servo_using_Joint, &_smart_servo, this});
                }

                for (int i = 0; i < jnt_num_; i++)
                    target_joint(i) = pos_[i];

                target_joint(index) = std::min(target_joint(index) + static_cast< double >( dir ) * max_speed * 0.1,
                                               joints_[index]->getMaxPosLimit());  //取最大速度的10%
                target_joint(index) = std::max(target_joint(index), joints_[index]->getMinPosLimit());
                _smart_servo.command(target_joint);

                break;

            case DraggingFlag::FLANG_X:
            case DraggingFlag::FLANG_Y:
            case DraggingFlag::FLANG_Z:
                ...
                if (_dragging_finished_flag) {
                    ...
                    thread_planning.reset(
                            new boost::thread{&JC_helper::smart_servo::smart_servo_using_Cartesian, &_smart_servo});
                    thread_IK.reset(new boost::thread{&JC_helper::smart_servo::smart_servo_IK, &_smart_servo, this});
                    thread_motion.reset(
                            new boost::thread{&JC_helper::smart_servo::smart_servo_motion, &_smart_servo, this});
                }

                index = index - static_cast< int >( DraggingFlag::FLANG_X );
            ...
            case DraggingFlag::TOOL_X:
            case DraggingFlag::TOOL_Y:
            case DraggingFlag::TOOL_Z:
                ...
                index = index - static_cast< int >( DraggingFlag::TOOL_X );
                x(index) = static_cast< double >( dir ) * max_speed * 0.1;
            ...
            case DraggingFlag::OBJECT_X:
            case DraggingFlag::OBJECT_Y:
            case DraggingFlag::OBJECT_Z:
                ...
                index = index - static_cast< int >( DraggingFlag::OBJECT_X );
                x(index) = static_cast< double >( dir ) * max_speed * 0.1;
