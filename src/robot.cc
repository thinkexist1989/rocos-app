...
namespace rocos
{
    Robot::Robot(HardwareInterface *hw,
                 ...
                 const string &tip) : hw_interface_(hw), urdf_file_path_(urdf_file_path), pos_(MAX_JOINT_NUM),
    ...
    void Robot::setEnabled()
    {
        ...
        for_each(joints_.begin(), joints_.end(),
                 [=](boost::shared_ptr<Drive> &d)
                 { d->setEnabled(false); }); // 将抱闸设置为同时开启，不阻塞
    ...
    outerloop:
        for (;;)
        {
            for (const auto &joint : joints_)
            {
                ...
                if (duration_us.count() > 150000)
                { // wait for 100ms  TODO: configuration_.driveStateChangeMaxTimeout
    ...
    void Robot::setDisabled()
    {
        ...
        for_each(joints_.begin(), joints_.end(),
                 [=](boost::shared_ptr<Drive> &d)
                 { d->setDisabled(false); }); // 将抱闸设置为同时开启，不阻塞
    ...
    int Robot::Dragging(DRAGGING_FLAG flag, DRAGGING_DIRRECTION dir, double max_speed, double max_acceleration)
    {
        ...
        if (dir == DRAGGING_DIRRECTION::NONE)
        {                      // 说明想要停止了
