...
Result MoveJog::Reset() {
    ...
    fk_target_        = Frame::Identity();
    clik_initialized_ = false;
...
JntArray MoveJog::computeJointVelocity() {
    ...
    if (is_joint_space_) {
        ...
        if (model_) {
            ...
            // 前馈项
            Eigen::VectorXd qd_ff =
                svd.matrixV() * inv_singular.asDiagonal()
                * svd.matrixU().transpose() * v;

            // ── CLIK 修正：递推目标位姿 → 误差 → J⁺·Kp·e ──
            if (!clik_initialized_) {
                model_->ForwardKinematics(q_integral_, fk_target_);
                clik_initialized_ = true;
            } else {
                // 位置递推
                fk_target_.p += KDL::Vector(tw.vel.x(), tw.vel.y(), tw.vel.z()) * s * dt_;
                // 姿态递推：Rot 内部会归一化轴，angle 须乘 tw.rot 的模长
                const double rot_norm = tw.rot.Norm();
                if (rot_norm > 1e-12) {
                    fk_target_.M = KDL::Rotation::Rot(tw.rot, rot_norm * s * dt_)
                                   * fk_target_.M;
                }
            }
            ...
            // 误差 = target ⊖ actual
            Frame fk_cur;
            model_->ForwardKinematics(q_integral_, fk_cur);
            KDL::Vector pos_err = fk_target_.p - fk_cur.p;
            KDL::Rotation R_err = fk_target_.M * fk_cur.M.Inverse();
            KDL::Vector rot_err = R_err.GetRot();

            Eigen::VectorXd v_err(6);
            v_err << pos_err.x(), pos_err.y(), pos_err.z(),
                     rot_err.x(), rot_err.y(), rot_err.z();

            Eigen::VectorXd qd_corr =
                svd.matrixV() * inv_singular.asDiagonal()
                * svd.matrixU().transpose() * (kp_ * v_err);

            for (unsigned int i = 0; i < q_dot.rows(); ++i)
                q_dot(i) = qd_ff(static_cast<int>(i))
                         + qd_corr(static_cast<int>(i));
        }
    }
    return q_dot;
}
