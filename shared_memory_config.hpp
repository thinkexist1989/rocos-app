...
public:
    ...
    ~SharedMemoryConfig() {
        if (ecm_base_ != nullptr) {
            if (munmap(ecm_base_, ecm_size_) != 0)
    ...
    // -----------------------------------------------------------------------
    /// @brief 在 mmap 后的段中扫描定位 EcatBus
    /// boost::managed_shared_memory 段首有内部管理头，EcatBus 位于头之后。
    /// 通过检查 slave_num、current_state 等特征字段定位，不依赖 boost header。
    static EcatBus* scanForEcatBus(void* addr, std::size_t size) {
        ...
        for (std::size_t off = 0; off <= max_off; off += 8) {
            ...
            // 主站运行时 slave_num > 0 且 current_state ∈ {INIT,PREOP,SAFEOP,OP}
            if (c->slave_num > 0 && c->slave_num <= MAX_SLAVE_NUM
                ...
                // 额外校验：第一个 slave 的 input_var_num 应当合理
    ...
    bool getSharedMemory() {
        ...
        const std::string shm_name = toPosixName(ecmName);
        ecm_fd_ = shm_open(shm_name.c_str(), O_RDWR, 0666);
        if (ecm_fd_ < 0) {
            print_message("[SHM] Ec-Master is not running (" + shm_name + ": " + std::strerror(errno) + ")", MessageLevel::ERROR);
            umask(mask); return false;
        }
        ...
        // 主站用 boost::managed_shared_memory，EcatBus 不在 offset 0 — 扫描定位
        ecatBus = scanForEcatBus(addr, ecm_size_);
        if (ecatBus == nullptr) {
            print_message("[SHM] Ec-Master is not running (EcatBus not found in " + ecmName + ")", MessageLevel::ERROR);
            munmap(addr, ecm_size_); ecm_base_ = nullptr;
            close(ecm_fd_); ecm_fd_ = -1;
            umask(mask); return false;
        }
    ...
    bool getPdDataMemoryProvider() {
        ...
        // 客户端只打开已存在的 PD 共享内存，不创建
        pd_input_fd_ = shm_open(pd_in.c_str(), O_RDWR, 0666);
        if (pd_input_fd_ < 0) {
            print_message("[SHM] Ec-Master PD input not ready (" + pd_in + ": " + std::strerror(errno) + ")", MessageLevel::ERROR);
            return false;
        }
        struct stat st_in{};
        ...
        pd_output_fd_ = shm_open(pd_out.c_str(), O_RDWR, 0666);
        if (pd_output_fd_ < 0) {
            print_message("[SHM] Ec-Master PD output not ready (" + pd_out + ": " + std::strerror(errno) + ")", MessageLevel::ERROR);
            munmap(pdInputPtr, pd_input_size_); pdInputPtr = nullptr;
            close(pd_input_fd_); pd_input_fd_ = -1; return false;
        }
        struct stat st_out{};
    ...
    void    *ecm_base_   = nullptr;   // mmap 基地址（用于 munmap，ecatBus 可能不在偏移 0）
