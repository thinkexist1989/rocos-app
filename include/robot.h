...
class Robot {
 ...
 public:
  ...
  class PathEntry {
   public:
    enum MoveType { MOVE_J, MOVE_L, MOVE_P, MOVE_C };
  ...
  enum Synchronization { SYNC_NONE, SYNC_TIME, SYNC_PHASE };

  enum OrientationMode { UNCONSTRAINED, FIXED };
 ...
 private:
  ...
  void RunMoveJ(JntArray q, double speed = 1.05, double acceleration = 1.4,
                ...
                bool asynchronous = false);
  //实际movel执行线程
  void RunMoveL(const std::vector<KDL::JntArray>&);
