...
double PULSE::pos(double time)
{
	...
	case VEL:		// 速度方波，则位移三角波
		...
		{
			ret = n * (_amp*(_cycle*(2.0*_ratio-1.0))) + _amp*(_cycle*_ratio) - _amp*(m-_cycle*_ratio);
		}
		else
		{
	...
	case ACC:		// 加速度方波,速度是三角波，位移是递增
		...
		if (m > _cycle*_ratio)
		{
			...
			ret = ret + v0*(_cycle*(m-_cycle*_ratio)) - 0.5*_amp*(_cycle*(m-_cycle*_ratio))*(_cycle*(m-_cycle*_ratio));
...
bool R_INTERP::planProfile()
{
	...
	case doubleS:
		...
		_doubleS->planDoubleSProfile(_tStart,_pStart,_pTarget,_vStart,_vTarget,_v_limit,_a_limit,_j_limit);
		break;
	...
	case polynomial_5th_T:
		...
		_pCurProfileType = _polynomial;
		_polynomial->plan5thProfileT(_tStart,_tTerminal,_pStart,_pTarget,_vStart,_vTarget,_aStart,_aTarget);
		break;
	...
	double v0, double vf, double v_limit, double a_limit)
{
	...
	if (_direction == 0)
	{
//		DEBUG("target and start points are the same!\n");
		return;
	}
...
{	
	...
	{
		DEBUG("trapezoid trajectory is not feasible\n");
		_plannedProfile = false;
		return ;
	}
	else
...
double Trapezoid::scaleToDuration(double newDuration)
{
	...
	_a[1] = -(1.0*(2.0*p0 - 2.0*pf + 2.0*Ta*v0 + Td*v0 + 2.0*Tv*v0 + Td*vf))/(Ta*(Ta + Td + 2.0*Tv));
	...
	double v0, double vf, double v_limit, double a_limit, double j_limit)
{
	...
	if (Tjx == a_limit / j_limit)
	{
		...
		{
			_plannedProfile = false;
			DEBUG("The two points are two close! DoubleS trajectory is not feasible!\n");
			return;
		}
	}
	else
	...
	if ( Tv <= 0)
	{
		...
		while(1)
		{
			Ta_acc = a_limit/j_limit;
			Ta_dec = a_limit/j_limit;
			temp = (a_limit*a_limit*a_limit*a_limit)/(j_limit*j_limit) + 2.0*(v0*v0+vf*vf)
				    + a_limit*(4.0*(pf-p0)-2.0*a_limit/j_limit*(v0+vf));
			...
			if ( Ta < 0 || Td <0)
			{
				...
				if ( Ta < 0)
				{
					...
					Ta_dec = (j_limit*(pf-p0) - sqrt(j_limit*(j_limit*(pf-p0)*(pf-p0) +
						(vf+v0)*(vf+v0)*(vf-v0) )))/(j_limit*(vf+v0));
					Ta = 0.0;
					Ta_acc = 0.0;
					break;
				}
			...
			else
			{
				if ((Ta<2*Ta_acc) || (Td<2*Ta_dec))
				{
					r = r - 0.01;
					a_limit = r*a_limit;
					//DEBUG("DoubleS re-planning: Ta<2*Tj1 or Td<2*Tj2\n");
				}
				else
				{
					break;
...
double DoubleS::pos(double t) const
{
	...
	{
		p = pf - (v_lim + vf)*Td/2.0 + v_lim*(dt-T+Td) + jmin*(dt-T+Td)*(dt-T+Td)*(dt-T+Td)/6.0;
		//dt = dt - Ta - Tv;
	...
	{
		p = pf - (v_lim + vf)*Td/2.0 + v_lim*(dt-T+Td) + a_limd/6.0*(3.0*(dt-T+Td)*(dt-T+Td)-3.0*Tj2*(dt-T+Td)+Tj2*Tj2);
		//dt = dt - Ta - Tv - Tj2;
...
double Polynomial::pos(double t) const
{
	...
	double p = _coefficient[0]*t*t*t*t*t + _coefficient[1]*t*t*t*t + _coefficient[2]*t*t*t 
		+ _coefficient[3]*t*t + _coefficient[4]*t + _coefficient[5];
	return p;
}
...
double Polynomial::vel(double t) const
{
	...
	double v = 5*_coefficient[0]*t*t*t*t + 4*_coefficient[1]*t*t*t 
		+ 3*_coefficient[2]*t*t + 2*_coefficient[3]*t + _coefficient[4];
	return v;
}
...
double Polynomial::acc(double t) const
{
	...
	double a = 20*_coefficient[0]*t*t*t + 12*_coefficient[1]*t*t
		+ 6*_coefficient[2]*t + 2*_coefficient[3];
	return a;
}
...
double Polynomial::max_vel()const
{
	...
	case polynomial_5th_T:
		...
		{
			t1 = -b/(3.0*a) + pow((b*c/(6.0*a*a) - b*b*b/(27.0*a*a*a) - d/(2.0*a) + sqrt(temp)),1.0/3.0);
			t1 = t1 + pow((b*c/(6.0*a*a) - b*b*b/(27.0*a*a*a) - d/(2.0*a) - sqrt(temp)),1.0/3.0);
			t2 = t1;
			t3 = t1;
		}
